...
@section('content')
    ...
    <div class="panel panel-orange" id="vrBlock">
        ...
        <ul class="list-group" id="datAnnoyingSizes">
            ...
            <li class="list-group-item" style="color: #777777;">@if($cities!=null)@foreach($cities as $city)<a class="orangeLinks" href="javascript:submit({{$city->id}}, '0' ,'empty')">{{$city->name}}</a>@endforeach @else <a class="orangeLinks" href="/">Уся Україна</a> @endif <span id="yellowCircle">&#183;</span> {{ date('j.m.Y, H:i:s', strtotime($vacancy->created_at))}}</li>
